...
(function(){
    ...
    function initUploadType(){
        ...
        if( (uploadType == 'local' || isDirect == 'php') ){
            ...
            makeFileUrl = chunkUploadUrl + '/mkfile';
        }
    ...
    function getFilename(file) {
        var type  = editor.getOpt('uploadSaveType'),
        ...
        if( type == 'date' ){
            if( time != '' ){
                filename = path + new Date().Format(time) + '/'+ Date.parse(now)+randNumber+"."+file.ext;
            }else{
                filename = path + '/'+ Date.parse(now)+randNumber+"."+file.ext;
            }
        }else{
        ...
        initUploader: function () {
                ...
                switch (type) {
                    ...
                    case 'uploadStart':
                        ...
                        if( files.size >= videoMaxSize ){
                            ...
                            uploader.option('server', uploadUrl);
                            uploader.option('sendAsBinary' ,false);
                            uploader.option('threads' ,1 );
                        }
                        break;
                    case 'stopUpload':
            ...
            uploader.on('uploadBeforeSend', function (file, data, header) {
                ...
                if( (uploadType == 'qiniu' &&  isDirect != 'php') || isChunked  ){
                    if( !isChunked ){
                        data['key'] = getFilename(file.file);
            ...
            uploader.on('uploadAccept',function(obj,ret){
                ...
                if( ret.offset == videoBlockSize ){
                    ...
                    // 创建块的url
                    url = makeBlockUploadUrl;
                    // 如果是最后一块,并且最后一块不能与videoBlockSize,设置创建块大小
                    if( $blockSize < videoBlockSize ){
                        url = chunkUploadUrl + '/mkblk/' + $blockSize;
                    }
                    qiniuCtx = qiniuCtx ? qiniuCtx + ',' + ret.ctx : ret.ctx;
                }else{
                    // 上传片url
                    url = chunkUploadUrl + '/bput/' + ret.ctx + '/' + ret.offset;
                    // 如果是最后一片，添加ctx
                ...
                try {
                    ...
                    if( isChunked ){
                        $.ajax({
                            ...
                            method   : 'post',
                            data     : qiniuCtx,
                            url      : makeFileUrl + '/' + file.size + '/key/'+ btoa(getFilename(file))+'/encodedMimeType/'+btoa(file.type),
                            success:function(data) {
                                console.log(data);
                                json = data;
                            }
                    ...
                    if( (uploadType == 'qiniu' &&  isDirect != 'php') || isChunked  ) {
                        if (typeof json.key != 'undefined') {
                            uploadVideoList.push({
                                'url': editor.getOpt('bucketUrl')+"/"+json.key
                            });
