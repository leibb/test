...
class YunXinImHelper
{
    ...
    public static function newInstance()
    {
        ...
        if ($environment == "debug") {
            return new static(
                env('YUNXING_APP_KEY_DEBUG'),
                env('YUNXING_APP_SECRET_DEBUG')
            );
        } else {
            return new static(
                config('yunxin.app_key'),
                config('yunxin.app_secret')
            );
        }
     ...
     * @return array $result      [返回array数组对象]
     */
    public function editUInfo($accid, $name = '', $icon = '', $gender = '0')
    {
        return $this->updateUinfo($accid, $name, $icon, $icon, '', '', '', '', $gender);
    }
     ...
     * @return array $result   200、403、414、416、431、500
     * @link https://dev.yunxin.163.com/docs/product/IM%E5%8D%B3%E6%97%B6%E9%80%9A%E8%AE%AF/%E6%9C%8D%E5%8A%A1%E7%AB%AFAPI%E6%96%87%E6%A1%A3/code%E7%8A%B6%E6%80%81%E8%A1%A8
     */
     ...
     * @param string $pushcontent [推送内容，发送消息（文本消息除外，type=0），option选项中允许推送（push=true），此字段可以指定推送内容。 最长200字节]
     * @param null $ext
     * @param null $payload
     * @return array $result      [返回array数组对象]
     */
    public function sendMsg($from
        ...
        , $type
        , $body
        , $option = array("push" => false, "roam" => true, "history" => true, "sendersync" => true, "route" => false)
        , $pushcontent = '', $ext = null, $payload = null)
    {
...
//dd($data);
     ...
     * @param string $pushcontent [ios推送内容，第三方自己组装的推送内容，如果此属性为空串，自定义通知将不会有推送（pushcontent + payload不能超过200字节）]
     * @param array $payload [ios 推送对应的payload,必须是JSON（pushcontent + payload不能超过200字节）]
     * @param string $sound [如果有指定推送，此属性指定为客户端本地的声音文件名，长度不要超过30个字节，如果不指定，会使用默认声音]
     * @return array $result      [返回array数组对象]
     */
    public function sendAttachMsg($from
        , $msgtype
        , $to
        , $attach
        , $pushcontent = ''
     ...
     * @param  $owner [群主用户帐号，最大长度32字节]
     * @param  $members [["aaa","bbb"](JsonArray对应的accid，如果解析出错会报414)，长度最大1024字节（群成员最多10个）]
     * @return array $result      [返回array数 组对象]
     */
